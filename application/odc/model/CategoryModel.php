<?php
	...
	{
		// 设置当前模型对应的完整数据表名称
		protected $table = '__ODC_CATEGORY__';
		...
		static public function getParentTrue($pam,$id = null)
		{
			...
			$data = $data_list->column('id,pid,name');

			$data_list = Tree::config(['title' => 'name'])->toList($data);
			$list = [];
			foreach ($data_list as $key => &$item)
			{
				$list[$item['id']] = $item['title_display'];
		 ...
		 * @param bool $del_last_url 是否Delete最后一个节点的url地址
		...
		static public function getTreeList($where = [])
		{
			return Tree::config(['title' => 'name'])->toLayer(self::where($where)->order('pid,id')->column('id,pid,name'));
